...
class BuildDataLoader:
    ...
    def __init__(self, folder):
        ...
        #with open('groundtruth/bldg_labeled_metadata.json'.format(self.folder), 'r') as fp:
            data = json.load(fp)
...
class ActivePartialLabelling(Inferencer):
    def __init__(self, source, budget, method, strategy, window, cflag, testM, beta, tune):
        ...
        self.DATA2 = BuildDataLoader(DATA_PATH)
        data.shuffle(8)
    ...
    #second constructor (not here anymore, it has been erased in this script, present in the original one)
