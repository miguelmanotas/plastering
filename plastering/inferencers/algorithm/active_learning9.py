...
class active_learning():
    ...
    def __init__(self, fold, perc, rounds, n_cluster, fn, label, transfer_fn=[], transfer_label=[], pt_type=[]):
    ...
    def plot_confusion_matrix2(self, label_test, fn_test, pt_type):
        ...
        fn_preds_inverted=le.inverse_transform(fn_preds).tolist()
    ...
    def run_CV(self):
        ...
        for train, test in kf:
            if counter_==0:
                ...
                c1 = KMeans(init='k-means++', n_clusters=self.cluster_num, n_init=10, random_state=None)
                c1.fit(fn_train)
                sils=silhouette_samples(self.fn, c1.predict(self.fn))
                selec=[]
                for i in range(len(sils)):
                    if sils[i]>sorted(sils)[round((len(sils))*self.perc/100)]:
                        selec.append(i)
                len(selec)
                self.selec=selec

                fn_train = self.fn[selec]
